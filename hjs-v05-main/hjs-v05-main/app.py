...
# =============================================================================
# HJS v0.5 Core — JEP v0.6-aware Accountability Layer for AI Agents
# Based on draft-wang-hjs-accountability-05 and aligned with JEP v0.6
# =============================================================================
...
# =============================================================================
# HJS v0.5 JEP v0.6 Archive / Evidence Lifecycle Helpers
# =============================================================================

def algorithm_tagged_digest(content, alg="sha256"):
    """Return an algorithm-tagged digest string, compatible with JEP v0.6."""
    if isinstance(content, str):
        content = content.encode("utf-8")
    if alg != "sha256":
        raise ValueError("HJS v0.5 baseline supports sha256 only")
...
class HJSArchiveReceipt:
    """
    HJS v0.5 archive receipt for a JEP v0.6 event.

    The receipt records archival protocol metadata and evidence lifecycle
    references. It does not redefine JEP-Core signatures, event hashes,
    validation levels, or failure codes.
    """
    def __init__(self, jep_event, archive_actor="did:example:hjs-archive",
                 retention_policy="default-retention", redaction_manifest=None,
                 selective_disclosure_manifest=None, evidence_refs=None):
        self.jep_event = jep_event
        self.archive_actor = archive_actor
        self.receipt_time = now_unix()
        self.archive_time = now_unix()
        self.event_hash = compute_event_hash(jep_event)
        self.retention_policy = retention_policy
        self.redaction_manifest = redaction_manifest or {
            "mode": "none",
            "redacted_fields": [],
            "retained_hashes": []
        }
        self.selective_disclosure_manifest = selective_disclosure_manifest or {
            "mode": "none",
    ...
    def to_dict(self):
        return {
            ...
            "archive_actor": self.archive_actor,
            ...
            "selective_disclosure_manifest": self.selective_disclosure_manifest,
...
class HJSArchiveStore:
    """Minimal in-memory HJS v0.5 archive store for JEP events."""
    def __init__(self):
        self.events = {}
        self.receipts = {}

    def ingest_jep_event(self, jep_event, **kwargs):
        receipt = HJSArchiveReceipt(jep_event, **kwargs)
        self.events[receipt.event_hash] = jep_event
        self.receipts[receipt.event_hash] = receipt.to_dict()
        return receipt.to_dict()

    def get_receipt(self, event_hash):
        return self.receipts.get(event_hash)


class HJSReceiptValidator:
    """Validate HJS archive receipt structure and JEP event-hash binding."""
    def validate_archive_receipt(self, receipt, jep_event=None):
        required = ["hjs", "type", "profile", "jep_event_hash", "receipt_time", "archive_time"]
        missing = [k for k in required if k not in receipt]
        if missing:
            return False, {"code": "HJS_ERR_RECEIPT_MISSING_FIELD", "missing": missing}
        if receipt.get("hjs") != "0.5":
            return False, {"code": "HJS_ERR_UNSUPPORTED_VERSION"}
        if receipt.get("type") != "hjs.archive_receipt":
            return False, {"code": "HJS_ERR_UNSUPPORTED_RECEIPT_TYPE"}
        if jep_event is not None:
            expected = compute_event_hash(jep_event)
            if receipt.get("jep_event_hash") != expected:
                return False, {"code": "HJS_ERR_EVENT_HASH_MISMATCH", "expected": expected}
        return True, {
            "valid": True,
            "profile": "jep-profile:hjs-archive:0",
            "scopes": ["archival_integrity", "evidence_lifecycle"],
            "jep_event_hash": receipt.get("jep_event_hash")
        }
...
def generate_hjs_event(verb, who_raw, what_content, aud, ref_mode, ref_hash,
    ...
    """Generate HJS v0.5 event with privacy extensions"""
...
def generate_hjs_archive_receipt(event_json):
    """Generate HJS v0.5 archive receipt for a JEP/HJS event JSON."""
    if not event_json.strip():
        return "❌ Please provide event JSON"
    try:
        event_dict = json.loads(event_json)
        receipt = HJSArchiveReceipt(
            event_dict,
            evidence_refs=[{"type": "event", "hash": compute_event_hash(event_dict)}],
            redaction_manifest={
                "mode": "manifest",
                "redacted_fields": [],
                "retained_hashes": [compute_event_hash(event_dict)]
            },
            selective_disclosure_manifest={
                "mode": "audience-bound",
                "audience_bound": True,
                "disclosed_fields": ["jep", "verb", "who", "when", "what", "nonce", "ref"]
            }
        ).to_dict()
        return json.dumps(receipt, indent=2, ensure_ascii=False)
    except Exception as e:
        return f"❌ Archive receipt error: {str(e)}"

with gr.Blocks(
    title="HJS v0.5 — Accountability Layer for AI Agents",
    css=".contain { max-width: 1400px; margin: auto; }"
) as demo:
    gr.Markdown("""
    # HJS v0.5 — Accountability Layer for AI Agents
    ### Event Recording Layer with Machine Immutability + Optional Human Privacy
    
    This demo implements the core design principles of **HJS v0.5** draft:
    ...
    gr.Markdown("---")
    gr.Markdown("### 📦 HJS v0.5 JEP Archive Receipt")
    hjs_archive_btn = gr.Button("Generate Archive Receipt for Current Event", variant="secondary")
    hjs_archive_output = gr.Textbox(label="HJS v0.5 Archive Receipt", lines=12)
    hjs_archive_btn.click(
        generate_hjs_archive_receipt,
        inputs=hjs_event_json,
        outputs=hjs_archive_output
    )
    
    gr.Markdown("""
    ---
    ### Relationship Between HJS v0.5 and JEP
    
    | Dimension | HJS v0.5 | JEP (draft-wang-jep-judgment-event-protocol-06) |
    |-----------|----------|------------------------|
    ...
    > HJS v0.5 explicitly states *"built upon JEP"* (Section 3), but this is actually a transition document during the renaming process.
